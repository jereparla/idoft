...
pr_data = {
    'columns': [
        'Project URL', 'SHA Detected', 'Module Path',
        'Fully-Qualified Test Name (packageName.ClassName.methodName)',
        'Category', 'Status', 'PR Link', 'Notes'
    ],
    'Category': [
        'OD', 'OD-Brit', 'OD-Vic', 'ID', 'NOD', 'NDOD', 'NDOI', 'NDOI', 'UD'
    ],
    'Status': [
        '', 'Opened', 'Accepted', 'InspiredAFix', 'DeveloperFixed', 'Deleted',
        'Rejected', 'Skipped'
    ],
    'PR Link':
        re.compile(r'((https:\/\/github.com\/((\w|\.|-)+\/)+)(pull\/\d+))'),
    'Notes':
        re.compile(
            r'(https:\/\/github.com\/TestingResearchIllinois\/((idoft)|(flaky-test-dataset))\/issues\/\d+)|^$'
        )
}
...
def check_status_consistency(filename, row, i, log):
  ...
  if row['Status'] in ['Accepted', 'Opened', 'Rejected']:
    ...
    if not pr_data['PR Link'].fullmatch(row['PR Link']) or (re.sub(
        r'\/pull\/\d+', '',
      ...
      if row[
          'Project URL'] == 'https://github.com/apache/incubator-dubbo' and re.sub(
              r'\/pull\/\d+', '',
              row['PR Link']).casefold() == 'https://github.com/apache/dubbo':
        pass
      else:
  ...
  with open(file, newline='') as csvfile:
    ...
    if uncommitted_lines != [] or committed_lines != []:
      for i, row in enumerate(info):
        ...
        # The line is either (1) only uncommitted (needs to always be checked locally), (2) only committed (needs to always be checked in CI)
